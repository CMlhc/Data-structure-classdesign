...
BOOL MaintainCarInfo(void) {
	...
	if (n == 1) {
		...
		getchar();
		while(strlen(newcol->Num)!=6||(int)newcol->Num[0]<48||(int)newcol->Num[0]>57||panduan(newcol->Num))
        {
	...
	if (n == 2) {
		...
		if (c3 == NULL) printf("%s\n\n", "没有什么好修改的。");
			...
			if (c3 == NULL) printf("%s\n\n", "没有符合条件的车辆。");
				...
				scanf("%20s", c3->Num);
				getchar();
				while(strlen(c3->Num)!=6||(int)c3->Num[0]<48||(int)c3->Num[0]>57||panduan(c3->Num))
            {
            printf("非法输入！请重新输入！\n");
            printf("%s", "车牌号码：");
		    scanf("%20s", c3->Num);
